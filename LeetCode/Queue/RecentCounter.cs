...
namespace DataStructureAlgorithm.LeetCode.Queue
{
    ...
    inclusive range [t - 3000, t].It is guaranteed that every call to ping 
    ...
    {
        private Queue<int> qRequestTimes;

        public RecentCounter()
        {
            qRequestTimes = new Queue<int>();
        }

        public int Ping(int t)
        {
            qRequestTimes.Enqueue(t);
            while (qRequestTimes.Peek() < t - 3000)
            {
                qRequestTimes.Dequeue();
            }
            return qRequestTimes.Count;
        }
