...
{
    // Source: https://leetcode.com/problems/dota2-senate/solutions/6493862/simple-best-solution-by-iqbaldiit-t52j/
    /*
        ...
        Output: "Dire"
        Explanation: 
        The first senator comes from Radiant and he can just ban the next senator's right in 
        round 1. And the second senator can't exercise any rights anymore since his right 
        has been banned. And the third senator comes from Dire and he can ban the first 
     ...
     */
    public static class Dota2Senate
    {
        static string PredictPartyVictory(string senate)
        {
            Queue<int> qRadent=new Queue<int>();
            Queue<int> qDire = new Queue<int>();
            ...
            while (qRadent.Count > 0 && qDire.Count > 0)
            {
                int rIndex = qRadent.Dequeue();
                int dIndex = qDire.Dequeue();

                if (rIndex < dIndex)
                    qRadent.Enqueue(rIndex + senate.Length);
                else
                    qDire.Enqueue(dIndex + senate.Length);
            }

            return qRadent.Count > 0 ? "Radiant" : "Dire";
        }
        ...
        public static void Execute()
        {
            ...
            Console.WriteLine(PredictPartyVictory("DDDDRD"));
        }
    }
}
